...
<header>
    ...
    <?php
      if(!empty($_GET['id']))
      {
        ...
        $query_contact = mysql_query("Select * from contact Where person_id='$id'");    
        $query_salary = mysql_query("Select * from employee Where emp_id='$id'");// SQL Query
        $rowp = mysql_fetch_array($query_person);
        $rowa = mysql_fetch_array($query_address);
        $rowc = mysql_fetch_array($query_contact);
        $rows = mysql_fetch_array($query_salary);
 ...
 <br/>
 <br/>
    <form class="pure-form pure-form-aligned" action="editemployee.php" method="POST">
    <fieldset>
        ...
        <div class="pure-control-group">
            <label >Salary</label>
            <?php
            echo '<input type="number" step="any" value="'.$salary.'" name="salary">';
            ?>
        </div>
        <br>
        <h3>Add Address</h3>
